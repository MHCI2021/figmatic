...
Widget vectorDataToWidget(double pcntSize, Map<String, dynamic> data) {
  Rect windowRect =
      toWindowRect(positionData: data["positioning"], pcntSize: pcntSize);
  return positionedWrapper(
      windowRect: windowRect, child: _getVectorWidget(windowRect, data));
}
...
Widget _getVectorWidget(Rect windowRect, Map<String, dynamic> data) {
  ...
  return Container(
      width: double.infinity,
      height: double.infinity,
      decoration: FigmaStyles(data)
          .getDecoration(colorFromString(data['style']["color"])));
}
